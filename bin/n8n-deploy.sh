...
# every workflow under n8n/workflows/*.json into the running onym-n8n
...
REQUIRED_FILES=(
    "ssh-manager-agent.json"
    "github-manager-bot.json"
)
MISSING=()
...
echo "  $(green OK)  onym-n8n running; secrets present; ${#WF_FILES[@]} workflow(s) found"
...
Next: register the org-level GitHub webhook (one hook per workflow path):
  ./bin/gh-webhooks-sync.sh
Subscribed events: issues, pull_request, pull_request_review,
